...
class QuickCopyTools {
    ...
    private class ProgressTracker {
        ...
        init(progressCallback: ((Int64, Int64) -> Void)?) {
        ...
        private func notifyProgress() {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                callback(self.copiedBytes, self.totalBytes)
            }
        }
    }
    ...
    private class CopyContext {
        ...
        init(progress: ((Int64, Int64) -> Void)?, token: CancellationToken, tracker: ProgressTracker, destination: String) {
    ...
    // 工具函数
    private static func chooseBSize(fileSize: Int64) -> Int {
        switch fileSize {
    ...
    // 主复制函数
    static func copyWithPhysicalWrite(
        from sourceURL: URL,
        ...
        Total size: \(tracker.totalBytes)
        Copy time: \(String(format: "%.2fs", copyTime))
        Total time: \(String(format: "%.2fs", totalTime))
        Average speed: \(Int64(Double(tracker.totalBytes) / copyTime))/s
        ------------------
        """)
    }
